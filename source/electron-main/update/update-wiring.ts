...
export function createUpdateServiceWiring(deps: {
  ...
  readonly openReleasePage?: (url: string) => void | Promise<void>;
  ...
  const githubToken = (deps.env ?? process.env).SAND_GITHUB_TOKEN?.trim() || undefined;
  ...
  const options: SandUpdateServiceOptions = {
    ...
    getMinimumVersion: () => null, getReleaseTrackGate: () => resolveReleaseTrackGate(deps.getExperimentService()?.getDynamicConfig("sand_internal_release_track_override") ?? {}), hasLiveExperimentBootstrap: () => deps.getExperimentService()?.hasLiveStatsigBootstrap() === true, getFlagsAgeMs: () => deps.getExperimentService()?.getFlagsAgeMs(), onStatusChange: deps.emitStatus, log,
    ...(githubToken === undefined ? {} : { githubToken }), ...(deps.openReleasePage === undefined ? {} : { openReleasePage: deps.openReleasePage }),
