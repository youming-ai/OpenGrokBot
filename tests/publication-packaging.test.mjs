...
test("Router settings use the trusted backend and display recorded inference usage", async () => {
  ...
  assert.match(rendererPatch, /getOllamaConfig/);
  ...
  assert.match(providers, /http:\/\/localhost:11434/);
  assert.match(providers, /\/v1/);
  assert.match(providers, /ollamaExecutor/);
  assert.match(providers, /OLLAMA_MODEL/);
  assert.match(providers, /OLLAMA_HOST/);
  assert.match(providers, /toolCallStreaming: true/);
  assert.match(providers, /parameters: jsonSchema\(parameters\)/);
  assert.match(providers, /You are OpenGrokBot, a warm, concise desktop assistant/);
  assert.match(providers, /recordRoutedUsage\(provider, usage\)/);
  assert.doesNotMatch(providers, /ANTHROPIC_API_KEY|OPENAI_API_KEY/);
  assert.match(cursorSession, /Ollama-only: the hosted cursor branch below is unreachable/);
  assert.match(cursorSession, /createProviderPromptSession\(routedProvider\)/);
  assert.match(cursorBackend, /Ollama-only: the hosted cursor branch below is unreachable/);
  ...
  assert.match(turnShell, /Ollama-only: routed sessions always serve the local model/);
  assert.match(turnShell, /createProviderPromptSession\(inferenceProvider\)/);
  assert.match(coordinator, /method !== "sendPrompt"/);
  assert.match(coordinator, /String\(row\.provider\) !== "ollama"/);
  ...
  assert.match(coordinator, /runRoutedProviderText\(provider, messages/);
